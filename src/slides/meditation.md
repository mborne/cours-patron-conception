...
Il est plus simple d'appeler partout dans le programme un `user.age` plutôt qu'un `user.getAge()`...

---

### Intérêt de l'encapsulation (2/3)

Le problème, c'est qu'un an après avoir stocké des "ages" dans des fichiers, on aura envie de changer pour stocker plutôt des "dates de naissances" (invariant temporel) 

En présence de getters et setters, on aurait pu modifier la structure interne sans casser les appels à `user.getAge()` :
